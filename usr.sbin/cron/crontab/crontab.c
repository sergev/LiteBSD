...
checkmaxtabsize(FILE *fp, size_t maxtabsize)
{
	...
	if ((unsigned int)statbuf.st_size > (unsigned int)maxtabsize)  {
		warnx("%ju bytes is larger than the maximum size of %ju bytes",
		    (unsigned int)statbuf.st_size, (unsigned int)maxtabsize);
		return 0;
	}
	return 1;
}
...
poke_daemon(void) {
	struct timeval ts[2];
	(void) gettimeofday(&ts[0], NULL);
	ts[1] = ts[0];
	if (utimes(SPOOL_DIR, ts) == -1)
