#!/usr/bin/python
#
# Autobuild script for LiteBSD
#
import sys, string, os, subprocess, shutil, time, datetime, MySQLdb

BSD     = "/website/litebsd/build/litebsd-sources"
ARCHIVE = "/website/litebsd/build/master"
REPO    = "https://github.com/sergev/LiteBSD.git"
...
os.system("cd "+BSD+"/.. && zip -rq "+ARCHIVE+"/"+str(rev)+"/sources-"+DATE+".zip litebsd-sources -x .git \\*/.gitignore")

#
# (6) Build everything in BSD/admin/build directory.
#
print "--- Build everything"
os.system("make -C "+BSD+"/admin/build all")

#
# (7) Move log and zip files into ARCHIVE.
#
print "--- Move log and zip files"
os.system("mv "+BSD+"/admin/build/*.log "+ARCHIVE+"/"+str(rev))
os.system("mv "+BSD+"/admin/build/*.zip "+ARCHIVE+"/"+str(rev))
